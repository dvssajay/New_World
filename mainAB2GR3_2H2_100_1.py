...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/ANDHRA_Bandersnatch2GR3_2H2_100_Run1.pth')
...
wandb.init(project="New_World", config={
    ...
    "epochs": 200,
    "batch_size": 128,
    "model": "ab_2GR3_2H2_100",
},     name="CIFAR100_ANDHRA_Bandersnatch2GR3_2H2_1"  # Custom run name
)
...
def train(epoch):
    ...
    wandb.log({
        ...
        **{f"train_accuracy_model_{i+1}": 100. * correct_individual[i] / total_individual for i in range(2)}
    })
...
def test(epoch):
    ...
    wandb.log({
        ...
        **{f"test_accuracy_model_{i+1}": 100. * correct_individual[i] / total_individual for i in range(2)}
    })
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ANDHRA_Bandersnatch2GR3_2H2_100_Run1.pth')
        best_acc = acc
