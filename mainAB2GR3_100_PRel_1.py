...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/ANDHRA_Bandersnatch2GR3_100_PRel_Run1.pth')
...
wandb.init(project="New_World", config={
    ...
    "epochs": 200,
    "batch_size": 128,
    "model": "ab_2GR3_100_PRel",
},     name="CIFAR100_ANDHRA_Bandersnatch2GR3_PRel_1"  # Custom run name
)
...
def train(epoch):
    ...
    correct_individual = [0] * 8
    total_individual = 0

    for batch_idx, (inputs, targets) in enumerate(trainloader):
        inputs, targets = inputs.to(device), targets.to(device)
        optimizer.zero_grad()
        out111, out112, out121, out122, out211, out212, out221, out222 = net(inputs)

        # Calculate losses for each output
        loss1 = criterion(out111, targets)
        loss2 = criterion(out112, targets)
        loss3 = criterion(out121, targets)
        loss4 = criterion(out122, targets)
        loss5 = criterion(out211, targets)
        loss6 = criterion(out212, targets)
        loss7 = criterion(out221, targets)
        loss8 = criterion(out222, targets)
        
        # Combine losses and backpropagate
        loss =  0.125 * (loss1 + loss2 + loss3 + loss4 + loss5 + loss6 + loss7 + loss8)
        loss.backward()
        optimizer.step()
        ...
        outputs = [out111, out112, out121, out122, out211, out212, out221, out222]
    ...
    wandb.log({
        ...
        **{f"train_accuracy_model_{i+1}": 100. * correct_individual[i] / total_individual for i in range(8)}
    })
...
def test(epoch):
    ...
    wandb.log({
        ...
        **{f"test_accuracy_model_{i+1}": 100. * correct_individual[i] / total_individual for i in range(8)}
    })
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ANDHRA_Bandersnatch2GR3_100_PRel_Run1.pth')
        best_acc = acc
