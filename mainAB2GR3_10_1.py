...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/ANDHRA_Bandersnatch2GR3_10_Run1.pth')
...
wandb.init(project="New_World", config={
    ...
    "batch_size": 128,
    "model": "ab_2GR3_10",
},     name="CIFAR10_ANDHRA_Bandersnatch2GR3_1"  # Custom run name
)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ANDHRA_Bandersnatch2GR3_10_Run1.pth')
        best_acc = acc
