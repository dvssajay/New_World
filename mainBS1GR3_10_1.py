...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/baseline1GR3_10_Run1.pth')
...
wandb.init(project="New_World", config={
    ...
    "epochs": 200,
    "batch_size": 128,
    "model": "baseline1GR3_10",
},     name="CIFAR10_baseline1GR3_10_1"  # Custom run name
)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/baseline1GR3_10_Run1.pth')
        best_acc = acc
