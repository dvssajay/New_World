...
class AB_2GR3_2H1_100(nn.Module): 
  def __init__(self, num_classes):
    super(AB_2GR3_2H1_100,self).__init__()
    
    self.Conv1 =  nn.Sequential(nn.Conv2d(3, 64, kernel_size=3, stride =1,padding=1, bias=False), nn.BatchNorm2d(64), nn.ReLU(inplace=False))
    ...
    self.Act21 = nn.ReLU(inplace=False)
    self.Act22 = nn.ReLU(inplace=False)
  ...
  def forward(self,x):
    ...
    out1, out2 = self.Act1(out) # Splitting at level 1

    out1 = self.Res21(self.Conv21(out1)) # Branch 1
    out2 = self.Res22(self.Conv22(out2)) # Branch 2

    out1 = self.Act21(out1)
