...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/baseline1GR3_100_PRel_Run1.pth')
...
wandb.init(project="New_World", config={
    ...
    "epochs": 200,
    "batch_size": 128,
    "model": "baseline1GR3_100_PRel",
},     name="CIFAR100_baseline1GR3_100_PRel_1"  # Custom run name
)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/baseline1GR3_100_PRel_Run1.pth')
        best_acc = acc
